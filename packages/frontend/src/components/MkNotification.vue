...
<div :class="[$style.root, { [$style.contentVisibilityAuto]: contentVisibilityAuto }]">
	...
	<div :class="$style.tail">
		...
		<div>
			...
			<template v-else-if="notification.type === 'twitchLiveStreamStarted'">
				<span :class="$style.text" style="opacity: 0.6;">{{ i18n.ts._twitch.startedStreaming }}</span>
				<MkA :class="$style.text" :to="`/live/@${notification.user.username}`" :title="notification.title">
					<i class="ti ti-quote" :class="$style.quote"></i>
					{{ notification.title }}
					<i class="ti ti-quote" :class="$style.quote"></i>
				</MkA>
			...
			<MkA v-else-if="notification.type === 'feedbackReceived'" :class="$style.text" to="/admin/feedback">
				[{{ notification.feedbackType === 'bug' ? i18n.ts._feedback.typeBug : i18n.ts._feedback.typeFeature }}] {{ notification.title }}
			</MkA>
...
.icon_googleAuthExpired,
.icon_feedbackReceived {
	display: grid;
	align-items: center;
