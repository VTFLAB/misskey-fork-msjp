...
| 6 | 公開 FQDN (`stream.msjp.pro` 案) と WAN 公開ポリシー例外 | **実施・実機検証済み (2026-07-16、WI-0.5)**: FQDN `stream.msjp.pro` で WAN 公開完了。Cloudflare proxied A レコード → 上位ルーター 443 → OPNsense rdr (source cloudflare_v4 限定) → HAProxy 443 TLS 終端 (`is_ome_stream_host` ACL) → OME 3333 の経路を採用 (01 §7 原案の「3333/3334 HAProxy 素通し・専用 frontend」は不採用)。メディア (UDP 10000-10009 / TCP 3478) は上位ルーター→OPNsense WAN→192.168.1.111 の NAT で開通。`${PublicIP}` STUN 解決が二重 NAT 環境でも真のグローバル IP に正しく解決されることも実機確認 (詳細は 01 §7.5)。外部3拠点・Docomo/Softbank 実回線での視聴成功を確認。ICE candidate により origin のグローバル IP が視聴者に開示される点はユーザー承認済み | 解消済 (WI-0.5 完了) |
