...
- Fix: 配信視聴ページの PC 表示で、コメント欄の高さをビューポート基準の決め打ちからプレイヤー+情報パネルの実高さに合わせる方式に変更 (bsky-fork 独自)。コメント数が少ない配信で、チャット欄だけ画面いっぱいまで間延びし入力欄の下に大きな空白ができていた問題を解消
- Fix: デッキ UI 使用時、プロフィールの LIVE バッジおよびライブ配信一覧ページから配信視聴ページを開くと「デッキへ戻る」バナーが表示され配信の視聴の妨げになっていた問題を修正 (bsky-fork 独自)。該当リンクに `?zen` を付与し、配信視聴ページを常に単一ページ表示で開くように
