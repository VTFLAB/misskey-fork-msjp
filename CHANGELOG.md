...
- Fix: 配信視聴ページで、ペイン幅が狭い (スマホ・縦画面) 場合にプレイヤー上部・コメント欄下部の縦積みレイアウトへ切り替わらず、固定幅 360px のコメント欄がプレイヤーを圧迫していた問題を修正 (bsky-fork 独自)。判定を `window.innerWidth` ベースの ResizeObserver から、他のレスポンシブ分岐と同じ CSS `@container` クエリ (ペイン幅基準) に置き換えた
