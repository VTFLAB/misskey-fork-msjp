...
- Fix: 「www」など w の連続だけのコメント (末尾の w 連続を含む) が英語と誤判定され、表示用の日本語訳が付いてしまう問題を修正 (bsky-fork 独自)。言語判定 (detectJaEn) の前処理で笑いの w 連続 (全体または末尾、直前が英字の場合を除く) を除去し、翻訳キュー投入・翻訳表示の対象外にした
