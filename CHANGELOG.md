...
- Fix: 配信視聴ページのスマホ表示を、フルスクリーンアプリ的な固定レイアウトに修正 (bsky-fork 独自)。ペイン幅が狭い場合、ページ全体の縦スクロールをやめて画面サイズに高さを固定し、プレイヤー上部・コメント欄下部の縦積みへ切り替える (CSS `@container` クエリで判定)。あわせてネイティブのページヘッダー (タイトル表示) を非表示にし、プレイヤー下の情報パネルにのみタイトルを表示することで縦幅の無駄を解消。コメント欄は「履歴がスクロール・入力欄は下部固定」のレイアウトになり、スクロールしないと入力欄が見えない問題を解消
