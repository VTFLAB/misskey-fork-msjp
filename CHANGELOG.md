...
### Client
- Feat: 新しいバグ報告・機能要望が届いた際、モデレーター全員に通知 (`feedbackReceived`、プッシュ通知対応) を送るように (bsky-fork 独自)。通知から `/admin/feedback` へ直接遷移できる
- Feat: 管理画面にバグ報告・機能要望の一覧ページ (`/admin/feedback`) を追加 (bsky-fork 独自)。モデレーターが全ユーザーの報告を状態フィルタ付きで確認し、状態変更・報告者への返信ができる (返信は報告者本人の `/feedback` に表示される)
- Enhance: バグ報告・機能要望フォームの本文に種別ごとの記入テンプレート (発生した問題/再現手順/期待する動作 等) を初期表示するように (bsky-fork 独自)。タイトルには記入例のプレースホルダを表示。テンプレート未記入のままでは送信できない
- Feat: バグ報告・機能要望の受付ページ (`/feedback`) を追加 (bsky-fork 独自)。種別 (バグ報告/機能要望)・タイトル・本文・スクリーンショット (画像のみ・最大4枚) を送信でき、送信済みの報告は同ページで状態 (受付済み/対応中/対応済み/見送り) と運営からの返信を確認できる。ナビゲーションに「バグ報告・要望」を追加
- Enhance: MSJP配信プレイヤーの音量バーに現在の音量パーセントを常時表示するように (bsky-fork 独自)。あわせてコントロールバーを配信プレイヤー風に刷新: 音量バーはホバー/フォーカスで展開 (タッチ環境は常時展開)、塗りを白系に統一、中音量アイコン (50%未満) を追加、ボタンにホバー背景、グラデーションとフェードイン (スライドアップ) を強化
- Enhance: 地震速報のトースト・通知・履歴ウィジェットに報種別 (第一報 / 最終報 / 取消) を明示するように (bsky-fork 独自)。プッシュ通知のタイトルにも報種別が入り、同じ地震の最終報は第一報の通知を置き換える
- Fix: 配信コメントの二重投稿対策 (Enter 長押しのオートリピートで再送信しない) (bsky-fork 独自)
- Fix: OBS用ライブ字幕ソース (`/live/:acct/subtitles`) で翻訳結果の字幕が次の翻訳が到着するまで永遠に画面に残り続ける問題を修正 (bsky-fork 独自)。原因は翻訳行の FIFO ポンプ (`pumpTranslationQueue`) が、現在の翻訳の表示時間 (文字数 / cps を [minDur, maxDur] でクランプした秒数、従来どおり) が経過したあと、キューが空だと何もせず次のポンプ呼び出しを待つ構造になっていた点。原文行には `idleClear` ベースのフェードアウトが実装されていたが翻訳行には等価の後処理が無く、結果として最後の翻訳がフェードアウトせず残り続けていた。表示時間経過後に次の翻訳が待機中で無い場合は `st-fade-out` クラス (opacity 遷移 300ms) を付与してフェードアウトさせ、300ms 後に `textContent` を空にして完全に非表示にするよう変更。新しい翻訳の描画開始時・`clear` イベント受信時にはこの後処理タイマーをキャンセルし、フェードアウト中や消去後の到着も通常のポンプ経路で正しく描画される (race safety: タイマー発火時に `translationBusy` が true に戻っていれば消去を中止)
- Fix: 配信アーカイブ履歴の「Driveに再アップロード」が、録画ファイルが大きいと必ず「Unexpected token '<' ... is not valid JSON」エラーになる問題を修正 (bsky-fork 独自)。アップロード完了まで API 応答を同期で待つ設計だったため、転送に数分かかるとリバースプロキシが先にタイムアウトして HTML エラーページを返し、サーバー側では実際には成功しているのにクライアントには失敗と誤表示されていた。前提チェック後すぐ応答してアップロード本体はサーバー側でバックグラウンド継続する方式に変更し、画面側は完了までステータスをポーリングして成否を通知する。転送中は再アップロードボタンを無効化し、二重実行はサーバー側でもアトミックな状態遷移でガードする
- Fix: コメント読み上げの再生基盤を HTMLAudioElement から Web Audio API へ全面置換 (bsky-fork 独自)。環境要因 (メディア要素へ介入するブラウザ拡張等) により `audio.play()` が AbortError となり1件も再生されない事象が実地で発生したため、DOM のメディア要素・blob URL を使わない AudioContext + AudioBufferSourceNode の直接再生へ変更。再生完了は onended とデコード済み音声長ベースのタイマーで二重判定し、「前の読み上げが完了するまで次を開始しない」直列性を再生経路の構造で保証する。エンジンのハングでキューが止まらないよう合成リクエストに30秒タイムアウトを追加し、自動再生ポリシーでブロックされた場合は次のクリック/キー入力で自動復旧・警告表示するように。再生専用の Web Lock 排他層は撤去 (単一キューへの統合で不要化)。再生開始/終了ログには時刻 (HH:MM:SS.mmm) を含める
- Fix: コメント読み上げがほぼ同時に届いた複数コメントで二重再生されることがある問題を修正 (bsky-fork 独自)。読み上げ停止処理が実行中フラグを即座に戻していたため、走行中の再生ループと並行して2本目のループが起動しうるレースがあった。実行中Promise自体で単一実行を保証する方式に変更し、構造的に排除
- Enhance: コメント読み上げで、翻訳対象になり得ないコメント (「8888」「www」など英字を含まない/笑いのみのコメント) が訳文を最大20秒待ってから読み上げられていたのを、即時読み上げに変更 (bsky-fork 独自)。バックエンドの翻訳キュー判定 (detectJaEn) と同じ基準で「翻訳が来ることが無い」コメントを見分けて待機をスキップする
- Enhance: コメント読み上げで「w」の連続 (全角・半角・文字数不問、コメント全体または末尾) を「わらわら」、「8」の連続 (全角・半角、2文字以上、全体・先頭・末尾) を「ぱちぱちぱち」と読み上げるように (bsky-fork 独自)。英単語の一部 (wow等) や数値 (1888・8880円・88.88等) は巻き込まない
- Fix: コメント読み上げの実行時状態 (キュー・再生器・設定) を globalThis 上のページ単一シングルトンへ移動 (bsky-fork 独自)。ビルドのチャンク分割で読み上げモジュールの複製が同一ページに複数ロードされ、複製ごとに独立したキューが並走して読み上げが多重再生される根本原因が実地の診断ログ (世代マーカーが1ページで4回出力) で確定したため。モジュールがいくつ複製されても全複製が同一のキュー・再生器を共有する
- Fix: コメント読み上げの再生自体を Web Lock で排他し、同一ブラウザ内での同時再生を経路を問わず不可能にする最終保証を追加 (bsky-fork 独自)。あわせて実行中のコード世代と再生開始/終了をコンソールで確認できる診断ログを追加
- Fix: コメント読み上げ中の音声が鳴り終わる前に次の読み上げが始まる問題を修正 (bsky-fork 独自)。再生完了の判定に pause イベントを含めていたため、再生完了前の pause 発火を「完了」と誤認してキューが先へ進み、前の音声と重なることがあった。完了判定を「再生終了 (ended) または明示停止」のみに限定し、音声長ベースのウォッチドッグ (音声長+3秒) を最終保証として追加。音声の実再生時間が経過する前に次の読み上げが始まることは構造的に起こらなくなった
- Fix: コメント読み上げの音声重複対策を強化 (bsky-fork 独自)。視聴ページを複数タブ・ウィンドウで開いた場合に各ページが独立して読み上げて音声が重なるため、Web Locks による「読み上げオーナー」排他で同一ブラウザ内の発話を常に1ページに限定した (オーナーのページを閉じると自動で移譲)。あわせてコメントID による二度読み防止と、キュー排出漏れの自己回復を追加
- Enhance: コメント読み上げで100文字を超える長文は途中で打ち切り「以下略」を付けて読むように (bsky-fork 独自)
- Enhance: コメント読み上げで URL を「ユーアールエル省略」と読み替えるように (bsky-fork 独自、読み上げソフトの定番辞書「URL省略」に倣う)。また定番ネットスラング辞書 (kwsk→くわしく / wktk→わくてか / ktkr→きたこれ / gj→ぐっじょぶ / thx→さんくす / orz→がっくり / うp→あっぷ / おk→おっけー / 乙→おつ) を追加し、全角表記も NFKC 正規化で吸収する。いずれも英単語の一部は巻き込まない
- Fix: Twitch同時転送中、視聴者がプレイヤーを切り替えるとチャット履歴が消え、コメントが配信者にも他の視聴者にも見えない「別部屋」に投稿される問題を修正 (bsky-fork 独自)。同時転送中は OME セッションと Twitch 中継セッションが別々の streamId を持ち、チャットが表示中プレイヤーのセッションに紐づいていたため。`twitch/streams/show` が返す正規チャットセッション (`chatStreamId`) に常に接続するよう修正し、プレイヤー切替でチャットが張り替わらないようにした。あわせてサーバー側でも、Twitch 中継セッション宛てのコメント投稿 (ローカルユーザー / リモートゲスト) を本体 (OME) セッションへ付け替えるようにし、未更新のクライアントからの投稿も同じチャット欄に届くようにした
- Fix: 配信終了後、視聴ページ (/live/:acct/stream) がリロードされて「見つかりません」表示のまま取り残される問題を修正 (bsky-fork 独自)。配信終了ダイアログを閉じるとチャンネルページ (/live/:acct) へ自動遷移するようにした (視聴ページ分離以前の挙動の復元)
- Feat: 配信設定 (`/settings/streaming`) に「Twitch同時転送」設定を追加 (bsky-fork 独自)。Twitch連携済みかつ配信チャンネル有効のユーザーにのみ表示され、有効化時には視聴制限が「公開」へ強制変更される旨の確認ダイアログを表示する。有効な間は視聴制限セレクタを「公開」でロックする
- Feat: 配信アーカイブ用のGoogle連携 (Drive/YouTube) が失効した際、再連携を促す通知 (`googleAuthExpired`) を配信者本人に表示するように (bsky-fork 独自)。通知から配信設定 (`/settings/streaming`) へ直接遷移でき、プッシュ通知 (Service Worker) にも対応
- Feat: 配信アーカイブが YouTube の「12時間超で事後削除」仕様に巻き込まれて録画が復元不能になる事故を構造的に防ぐ多層対策のうち、視聴・配信者UX側の改善を追加 (bsky-fork 独自)。YouTube は動画長が12時間を超えるアップロードを事後に削除する仕様で、実際に13時間超の配信録画が YouTube へ正常にアップロードされ (videoId 発行・Misskey 側は成功記録) たにもかかわらず YouTube 側で事後に削除され、成功パスでは Google Drive コピーを作らずローカルファイルも削除していたため録画が復元不能になる事故が発生していた。視聴UX面では、アーカイブプレイヤー・アーカイブ一覧が `youtubeUploadStatus` の `unavailable` (YouTube 側で事後削除) / `skipped` (12時間超でYouTube対象外) の状態を正しく表示し、死んだ YouTube 埋め込みを描画しないようにした (`unavailable` は Drive コピーがあれば Drive 再生へフォールバック、なければアーカイブ利用不可を表示)。また失敗アーカイブ (Drive/YouTube いずれの永続コピーも確保できなかった場合) は TNAS 上に7日間保持し、配信アーカイブ履歴に警告と「保持期限まで残りN日」のカウントダウン、[録画をダウンロード] と [Driveに再アップロード] の操作を表示する。バックエンド側の対策 (録画長ゲート・事後削除検知・配信中事前警告・保持期限ジョブ) は `### Server` 側を参照
- Fix: 配信アーカイブのYouTube動画にサムネイルが表示されず、常に汎用アイコンのままだった不具合を修正 (bsky-fork 独自)。Google Drive側は`thumbnailLink`を保存・表示する仕組みがあったが、YouTube側は対応するDBカラム自体が存在せず、アップロード成功時のレスポンス (`snippet.thumbnails`) も読み取っていなかったため。YouTube Data APIのアップロードレスポンスからサムネイルURLを取得してDBに保存するようにした
- Feat: 配信アーカイブをMSJPインスタンス内で完結して視聴できるように (bsky-fork 独自)。YouTube側も埋め込みiframeでの再生に対応し、専用の視聴ページ (`/live/:acct/archive/:streamId`) で配信中のコメントを再生位置に同期して再現する「コメントリプレイ」機能 (YouTube視聴時) / 時系列一覧表示 (Google Drive視聴時) とあわせて視聴できるようにした。チャンネルホームのアーカイブ一覧・配信アーカイブ履歴からもこの専用ページへ遷移する (従来はYouTube側のみMisskey外の外部リンクだった)
- Feat: 配信アーカイブに配信終了時点の視聴制限 (公開/フォロワー限定/パスワード/ユーザー指定) を引き継げるように (bsky-fork 独自)。従来アーカイブには視聴制限が一切適用されておらず、限定公開の配信でもアーカイブは誰でも視聴できてしまっていた。配信終了時点の設定をスナップショットしてアーカイブ単位で保持し、配信アーカイブ履歴の設定画面から個別に変更できる。非認可ユーザーにはYouTube動画ID/Google DriveファイルID自体を返さない形で実効性を確保し、コメントリプレイの閲覧にも同じ制限を適用する
- Feat: 配信アーカイブ履歴に「公開を取り消す」機能を追加 (bsky-fork 独自)。YouTube/Google Drive上の実ファイルは削除せず、MSJP側での一覧掲載・視聴・コメントリプレイ閲覧のみを取り消す (一方向のみ、再公開はできない)
...
- Enhance: 配信設定 (`/settings/streaming`) に、ビットレート超過遮断で MSJP配信 (OME) の再配信が一時的にブロックされている間、配信者本人にその旨と理由・解除までの残り時間を警告表示するように (bsky-fork 独自)。従来は配信が始められない原因が分からず、単なる不具合と誤認されうる状態だった
- Fix: 配信視聴ページ (/live/:acct/stream) のプレビューモード (配信開始前のチャット動作確認) でコメント入力欄が完全に非表示になっていた不具合を修正 (bsky-fork 独自)。入力欄の表示条件に不要な「配信中であること」の判定が混入しており、プレビュー中は常に入力欄・匿名視聴者向けのゲストログイン導線ともに描画されなくなっていた
...
- Fix: 視聴ページの配信元切替をタイトル行のアイコンボタンに統合し、モバイルでの縦スペース消費を削減 (bsky-fork 独自)。プレイヤー直下の常時表示行だった専用セクションを廃止し、タイトル・設定ボタンと並ぶアイコンボタンから切替メニューを開く方式に変更
...
- Fix: リモートゲストログイン中も、配信視聴ページの画面端に未ログイン (visitor) 向けの登録案内パネル (サーバー説明・「このサーバーに登録する」等) が表示され続けていた問題を修正 (bsky-fork 独自)。リモートゲストとして既にログイン済みのため、登録・ログインへの誘導は不要と判断し非表示にした
- Feat: ノート投稿フォームに MFM 構文挿入ツールバー (bsky-fork 独自) を追加
...
- Enhance: サイドメニューのデフォルト項目を「お知らせ」から「アップデート情報」に変更 (bsky-fork 独自)。「お知らせ」は設定 (`/settings/navbar`) から引き続き追加可能
- Enhance: 天気予報ウィジェットのデザインを改良 (bsky-fork 独自)。体感温度・本日の最高/最低気温・降水確率を追加表示し、気温表示とアイコンを大きく、夜間はアイコンをやや控えめに表示するように
...
- Enhance: 配信視聴ページのコメント投稿フォームをノート投稿と同等のエディタに強化 (bsky-fork 独自)。MFM 構文挿入ツールバー・MFM 構文 (太字・打消し・関数など)・カスタム絵文字・オートコンプリート (メンション・ハッシュタグ・絵文字)・絵文字ピッカーに対応し、ドライブファイルの添付・ドラッグ&ドロップ・クリップボードペーストでのメディアアップロードが可能に。コメント表示に投稿時刻 (HH:mm・ツールチップにフル日時) を追加
- Fix: 配信視聴ページのスマホ表示を、フルスクリーンアプリ的な固定レイアウトに修正 (bsky-fork 独自)。ペイン幅が狭い場合、ページ全体の縦スクロールをやめて画面サイズに高さを固定し、プレイヤー上部・コメント欄下部の縦積みへ切り替える (CSS `@container` クエリで判定)。コメント欄は「履歴がスクロール・入力欄は下部固定」のレイアウトになり、スクロールしないと入力欄が見えない問題を解消
- Fix: 配信視聴ページの視聴者数表示を削除 (bsky-fork 独自)。5 分間隔ポーリングでしか更新されずリアルタイム性がなく、表示上「カウントが機能していない」ように見えるため
...
- Fix: デッキ UI 上でリンクをポップアップウィンドウ (`MkPageWindow`) として開いた場合に配信視聴ページの高さ計算が崩れ、チャット履歴がスクロール領域にならず入力欄が見切れる問題を修正 (bsky-fork 独自)。高さ基準を `100dvh` (ブラウザビューポート) から `100cqh` (コンテナクエリ、`RouterView.vue` の `_pageContainer` を基準) に変更し、通常ページ・ウィンドウ双方で実際の表示枠に合わせて高さが決まるようにした。あわせてモバイルフッターナビ分の差し引き (`--MI-minBottomSpacing`) を廃止 (100cqh 化により `_pageContainer` の時点で既にフッター分が除かれているため、差し引くと二重減算になり下部に余白ができていた)
- Fix: 配信視聴ページのスマホ/PC レイアウト切り替えを、ペイン幅だけでなく縦横比でも判定するように変更 (bsky-fork 独自)。固定幅 360px のコメント欄を確保したまま 2 ペインで並べると、正方形〜縦長のペイン (縦置きモニタ、デッキのポップアップウィンドウを正方形寄りにリサイズした場合など) ではプレイヤーが極端に細長い領域に押し込まれ、Twitch 埋め込みプレイヤーが上下に大きくレターボックスされて実質的な表示サイズが小さくなる問題があった。ペインが少しでも縦長 (aspect-ratio が 1:1 未満) になったら、幅に余裕があっても縦積みレイアウトへ切り替えるようにした
...
- Fix: 配信視聴ページで、リモートゲストが投稿したコメント内のカスタム絵文字コードが画像として描画されず、テキストのまま表示される問題を修正 (bsky-fork 独自)。挿入される絵文字は投稿者のインスタンスに関わらず当サーバーのローカル絵文字のため、表示側もローカル絵文字として解決するようにした
- Fix: 配信視聴ページで、リモートゲストのコメントに常に汎用アイコンが表示され、実際のアバター画像が反映されない問題を修正 (bsky-fork 独自)。ゲストアカウントに保存済みのアバター URL をコメント一覧・リアルタイム配信のレスポンスに含めるようにした
...
- Feat: 配信者が配信開始前でも自身の配信視聴ページでチャット動作確認ができるプレビューモードを追加 (bsky-fork 独自)。オフライン表示中に「プレビューモードで開く」ボタンから通常の視聴レイアウト・チャットを開ける (投稿・翻訳・読み上げ等も動作確認できる)。オフライン中の Twitch チャットの発言もプレビューのチャット欄に取り込まれる。プレビュー中は情報パネルにその旨を表示し、配信中/未配信の一覧・LIVEバッジには一切影響しない
- Feat: OBS のブラウザソースに貼れる、見た目をカスタマイズ可能なコメントジェネレーターページ (`/live/@username/comment-generator`) を追加 (bsky-fork 独自)。既存のコメントオーバーレイページとは別に、Misskey フロントエンド (Vue/vite) と完全に隔絶した軽量スタンドアロン HTML/CSS/JS で実装。URL クエリパラメータで表示モード (フェードアウト/積み上げ)・新着コメントの表示順 (上/下)・同時表示件数・アニメーション・フォント・配色・アイコン/名前/翻訳/添付画像の表示可否・絵文字サイズ等を細かく調整でき、色・フォント等は CSS カスタムプロパティ (`--cg-*`) 経由のため OBS 側のカスタム CSS でも上書き可能。設定ビルダー画面 (`/settings/twitch` の配信者設定から開くモーダル) には組み込みプリセット・ユーザー保存テンプレートの選択/上書き/削除、設定 JSON のエクスポート/インポート機能も備え、実配信を待たずに見た目を確認できるダミーコメント自動生成 (`demo=1`) によるライブプレビュー用 iframe も内蔵する
- Fix: コメントジェネレーター設定ダイアログのライブプレビュー (iframe) が常に空白になり、アニメーション・積み上げ表示 (`mode=stack`) が一切確認できない問題を修正 (bsky-fork 独自)。`/live/:acct/comment-generator` がクリックジャッキング対策のグローバル `X-Frame-Options: DENY` を外していなかったため、ブラウザがプレビュー用 iframe への埋め込みを拒否していた。`/embed/*` と同様にこのルートでも `X-Frame-Options` を除去し、プレビューが表示されるようにした (OBS ブラウザソースとして直接開く本番の埋め込み先は元々フレームではないため影響を受けていなかった)
- Fix: コメントジェネレーター設定ダイアログで、組み込みプリセット/保存済みテンプレートを選び直しても既に選択中のものと同じだとフォーム値が更新されない問題を修正 (bsky-fork 独自)。`MkSelect` の `v-model`/`defineModel` は選択値が変化しない場合 `update:modelValue` 自体を発火しない仕様のため、テンプレート選択を専用の `os.popupMenu` に置き換え、クリックのたびに確実に再適用されるようにした
- Enhance: コメントジェネレーター設定ダイアログにテンプレート切替時の未保存変更確認・明示的な「プレビューに適用」ボタン・フォント選択 (汎用フォントプリセット + Driveのカスタムフォント指定) ・色設定のカラーピッカー (背景色は不透明度スライダー・透過スイッチ付き) を追加 (bsky-fork 独自)。フォーム編集は下書きのみに反映され、適用ボタンを押すまで iframe プレビュー・生成URL・保存内容には反映されない。テンプレート未保存の変更がある状態で別のテンプレートを選ぶと確認ダイアログを表示。Drive のカスタムフォントは新規 URL パラメータ `fontUrl` (http/https のみ) 経由で配信され、フォントジェネレーター側 (`comment-generator.js`) で `FontFace` を用いて読み込む (読み込み失敗時は既存のフォールバックフォントで通常動作)
- Enhance: コメントジェネレーター設定ダイアログの反映段階を、フォーム (下書き) → プレビュー → 本番用URL の3段階に拡張 (bsky-fork 独自)。「プレビューに適用」は iframe プレビューのみを更新し、新設の「設定を反映」ボタンではじめて生成URL表示・URLコピー・miLocalStorage への永続化 (OBSで実際に使う本番URLの元) に反映されるようにした。「設定を反映」はプレビュー内容と反映済み内容に差分があるときのみ活性化し、フォームにまだプレビュー未適用の変更が残っている場合はその旨のヒントを表示する。反映されていない変更が残ったままダイアログを閉じようとすると確認ダイアログを表示する
- Feat: コメントジェネレーターにコメントバブルの横位置設定 (`align`: 左/中央/右寄せ、デフォルト左) を追加 (bsky-fork 独自)。従来はコンテナ幅いっぱいにバブルが伸びて常に左端にしか見えなかったため、`.cg-comment` を fit-content 幅に変更したうえで `align-items` で寄せを切り替えるようにした。設定UI上は既存の縦位置 (新着コメントが上/下) と並べて「表示位置」としてまとめて配置
- Fix: OME 視聴プレイヤー (MkOmePlayer) で変更した音量がページ更新後に保持されず、ミュート解除時に常に既定の 50% に戻ってしまう不具合を修正 (bsky-fork 独自)。MkMediaRange が `<input type="range">` の値を文字列で書き戻すため localStorage に文字列が保存され、復元時の `typeof === 'number'` 判定で弾かれていた。音量判定・演算を数値化した単一ソース (`volumeNum`) 経由に統一し、保存・復元とも数値/数値文字列の両方を受理するようにした
- Enhance: OME 視聴プレイヤーのコントロール操作性をスマホで改善 (bsky-fork 独自)。ホバーの無いタッチ環境 (`@media (hover: none)`) ではコントロールバーを常時表示にし、音量スライダー・ミュート/全画面ボタンのタッチターゲットを拡大した。PC (ホバー可能環境) の既存オートハイド挙動は変更なし
- Feat: ライブチャンネルに「オフライン画像」設定を追加 (bsky-fork 独自)。配信者が設定した画像を、配信停止・再接続中に OME 視聴プレイヤー上へ表示し、配信終了直後にプレイヤーのエラー画面が出る体験を解消する。未設定時は従来どおり再接続表示にフォールバック。設定は配信設定ページ (`/settings/streaming`) の banner と同じ要領でアップロード/ドライブ選択できる
- Enhance: 配信視聴ページのページ再取得ボタンを視聴者にも常時表示するように (bsky-fork 独自)。従来はプレビュー中・配信者のみに限定されていたが、不具合時などに視聴者が任意でページを再取得できるようにした
- Enhance: 配信視聴ページをスマホで開いた際に、グローバルなモバイルフッターメニュー (ホーム・通知等) を非表示にして視聴に集中できるように (bsky-fork 独自)。ページ側からモバイルフッターを隠せる仕組み (`PageMetadata.hideMobileFooter`) を追加した
- Enhance: ライブ配信ページ (`/live`) を「配信チャンネル」一覧へ再構築 (bsky-fork 独自)。開設済みの MSJP配信チャンネルをカード表示 (配信中バッジ・バナー/オフライン画像・チャンネル名・説明・配信者) し、未ログインでも閲覧可能に。従来の Twitch 中継一覧はタブ切り替えで統合 (デフォルトタブは「配信チャンネル」)
...
- Fix: 配信視聴ページ・配信アーカイブ視聴ページ・OBS用オーバーレイが、デフォルトUI (universal) や未ログイン (visitor) UI ではサイドバー・ウィジェット・お知らせなどの Misskey グローバルUIが表示されたままになり、視聴の邪魔になっていた不具合を修正 (bsky-fork 独自)。デッキUIでは既存の zen 自動降格の仕組みにより意図通り表示されていたが、他のUIモードには同等の仕組みが無かった。ページ側からグローバルUIを一括で隠せる仕組み (`PageMetadata.immersive`) を追加し、該当ページに適用した
- Fix: 配信アーカイブ視聴ページのYouTube埋め込みプレイヤーの初期音量が100% (最大) になっていた問題を修正 (bsky-fork 独自)。IFrame Player APIの`setVolume`で初期値を50%に変更。Google Drive埋め込み側は外部プレイヤーが音量制御用のAPIを公開しておらず技術的に対応不可のため、視聴者がDrive内蔵の音量スライダーで調整する現状のまま
- Fix: 配信アーカイブ視聴ページのGoogle Drive埋め込みプレイヤーが、モバイル等の狭い画面幅 (実効幅約420px未満) で内部レンダリングが崩れ、映像が見切れる問題を修正 (bsky-fork 独自)。Google Drive `/preview` 埋め込みは一定の実効幅を下回ると崩れる既知の制約があるため、iframeを常に十分な基準幅 (600px) で描画しCSS `transform: scale()` で実際の表示幅に縮小する方式に変更
- Fix: 配信視聴ページ (`/live/:acct/stream`) で、配信者が自分の配信ページを未配信 (オフライン) 状態またはプレビューモード中に開くと、プレイヤー領域が黒背景のまま何も表示されない不具合を修正 (bsky-fork 独自)。ライブ中でない場合の表示分岐が抜けており空のまま残っていたため、チャンネルの「オフライン画像」が設定されていればその画像を、無ければ「現在オフラインです」のメッセージ (アイコン付き) を表示するようにした
...
### Server
- Feat: モデレーター向けフィードバック管理 API `admin/feedback/list` / `admin/feedback/update` を追加 (bsky-fork 独自)。権限 `read:admin:feedback` / `write:admin:feedback` を新設
...
- Enhance: 地震速報のプッシュ通知を即時送信するように (bsky-fork 独自)。通知基盤の「アプリ内で2秒以内に既読にならなければプッシュする」既読待ちを地震速報に限りバイパスし、受信から即時にプッシュ通知を送出する。あわせて Wolfx 発表時刻と受信時刻の差 (上流配信遅延) をログに記録し、配信ソース追加 (P2P地震情報 / dmdata 並行購読) の判断材料にする
- Feat: LAN 内の自動化クライアント (ローカル LLM / MCP 等) からアップデート情報を認証不要で投稿できる `update-info/create-local` エンドポイントを追加 (bsky-fork 独自)。X-Forwarded-For 付きリクエスト (= 公開プロキシ経由) は無条件拒否し、config `updateInfoLocalPost.allowedIps` に列挙した送信元のみ許可する二重ガード。未設定なら機能オフ
- Enhance: 地震速報の履歴・通知・トーストを「1つの地震につき第一報と最終報 (と取消報) のみ・最大予測震度3以上」に絞るように (bsky-fork 独自)。従来は全ての続報が履歴に載り、警報級では続報のたびに通知が重複して通知欄が埋まっていた
- Fix: 配信コメント投稿の冪等性確保 (レスポンス消失時の再送で同一コメントが重複作成されないように Redis ベースの重複排除を追加) (bsky-fork 独自)
- Fix: OBS 5500kbps CBR 配信が30秒の受信バースト (8832kbps) で誤切断された 2026-08-09 の事象を受け、ビットレート超過判定の窓を 3サンプル (30秒) → 6サンプル (60秒) に拡大した (bsky-fork 独自)。録画バイト数実測でセッション平均 5504kbps と設定準拠が確認されており、バーストは回線再送等の一時的要因と判定。あわせて閾値の8割に到達したサンプルが出た場合に事前警告ログ (5分に1回以内) を出すようにした
- Fix: retention からの Drive 再アップロード成功後、ローカルの録画 mp4 が削除されず NAS 上に残り続ける問題を修正 (bsky-fork 独自)。DB 上のパス参照だけがクリアされて retention cleanup の追跡からも外れ、孤児ファイルとして蓄積していた
- Fix: 配信アーカイブのアップロードが Google 側の一時的な障害 (HTTP 408/429/5xx やネットワーク断) で即座に失敗確定し、アーカイブが失われる問題を修正 (bsky-fork 独自)。実際に Google の一時 408 (Request Timeout) で「YouTube・Drive とも失敗、7日間の retention 行き」が発生したことへの対策3点: (1) Drive/YouTube へのアップロードを一時エラー時に最大3回まで指数バックオフでリトライする (ストリームは試行ごとに作り直す)。(2) YouTube がクォータ超過以外の理由で失敗した場合 (一時エラーのリトライ枯渇・権限失効など) も Drive へのフォールバック保存を必ず実行する。従来はクォータ超過のみフォールバックし、それ以外は Drive 連携済みでも永続コピーゼロのまま retention 行きだった。(3) YouTube リトライキューも一時エラーでは失敗確定させず queued を維持して次の1時間後の実行で再試行する。あわせて、エラー表示に Google の HTML エラーページが丸ごと保存・表示されていたのを「HTTP 408」のような1行要約に変更
- Fix: 「www」など w の連続だけのコメント (末尾の w 連続を含む) や URL だけのコメントが英語と誤判定され、表示用の日本語訳が付いてしまう問題を修正 (bsky-fork 独自)。言語判定 (detectJaEn) の前処理で笑いの w 連続 (全体または末尾、直前が英字の場合を除く) と URL を除去し、翻訳キュー投入・翻訳表示の対象外にした
- Fix: 規定内のビットレートで配信していても、長時間配信中にビットレート超過として誤切断されることがある問題を修正 (bsky-fork 独自)。OME の瞬間ビットレート計測 (bitrateLatest) は WebRTC のキーフレーム一括送出の影響で、平坦な CBR 配信 (実測: 7500kbps CBR + nal-hrd=cbr で 5888〜8869kbps) でも ±20% 程度振れる。従来の「瞬間値が閾値 (上限+10%) を3回連続で超過」判定はこの計測ノイズだけで確率的に成立してしまうため、「直近3サンプル (約30秒) の平均が閾値超過」の判定に変更。真に上限超過している配信は従来どおり約30秒で遮断される
- Enhance: Twitch EventSub を WebSocket transport から webhook transport へ全面移行 (bsky-fork 独自)。従来は bot + 連携配信者ごとの常駐 WebSocket セッション構成で、Twitch 起点の session_reconnect・プロセス再起動のたびに購読の張り直し窓が生じて stream.online/offline を取り逃す (最大2分のポーリングまで検知が遅延する) ほか、配信開始/終了の検知が配信者本人のトークン健全性に依存していた。webhook 化により購読は Twitch 側へ永続化され、全購読を app access token で作成するため配信終了 (stream.offline) がトークン失効・再接続と無関係に即時届く。サーバー側の受け口は `POST /twitch/eventsub` の1エンドポイント (HMAC-SHA256 署名検証 + Redis 重複排除) のみで、常駐接続が連携人数に比例して増えない (webhook の購読上限は max_total_cost=10,000、連携配信者は client 認可済みのため実コスト0)。あるべき購読集合と Twitch 側の実購読は60秒間隔で突合し、連携/解除の反映と失敗購読 (検証失敗・配送失敗・認可取り消し) の自動復旧を行い、購読が欠けていた期間は Helix Get Streams で即時自己修復する。2分間隔の全件ポーリングは最終防衛線として存置
- Fix: Twitch同時転送中のチャット中継がMSJP配信 (OME) のチャット欄と連動していなかった問題を修正 (bsky-fork 独自)。チャット中継は従来Twitch中継セッション (source=twitch) 前提で実装されており、同時転送中のTwitch側チャットは視聴者が見ているOME配信のチャット欄に現れず、MSJP側コメントもTwitchへ送信されなかった。今後は同時転送有効かつOME配信中の場合、(1) Twitch側チャットはOME配信のチャット欄へ取り込み (コメントリプレイにも合流)、(2) MSJP側コメントは本人の連携アカウントのTwitchチャンネルへbotが中継発言する
- Fix: Twitch EventSub の購読上限超過 (429 "total cost exceeded") により、連携配信者が6人以上いると一部配信者の配信開始/終了検知がポーリングフォールバック頼みになっていた問題を修正 (bsky-fork 独自)。EventSub WebSocket の上限 (コスト合計10) は「トークン所有者ごと」に適用されるため、従来の bot トークン1セッション集約 (stream.online/offline がコスト1×2×人数) をマルチセッション構成に分離: bot セッションは全配信者のチャット購読のみ (コスト0)、各配信者の配信開始/終了は本人の連携トークンによる個別セッションで購読する (自分のチャンネルなのでコスト0、scope 不要のため再連携も不要)。これにより全購読がコスト0となり連携人数の上限が構造的に消滅する。配信者トークン失効時はそのセッションのみ停止し、従来どおり2分ポーリングが検知を代替する
- Feat: MSJP配信 (OME) のTwitch同時転送 (bsky-fork 独自)。Twitch連携済みユーザーが任意に有効化でき、配信開始時にTwitchのストリームキーをHelix API (`channel:read:stream_key` scope、機能追加前の連携ユーザーは再連携が必要) から都度取得し (DB保存しない)、OMEのPush Publisher経由でRTMP転送する。映像はバイパス (H.264 再エンコードなし)、音声はTwitchのRTMP要件に合わせてOpus→AAC変換したレンディションを使用。Twitch側では視聴制限をかけられないため、有効化時に視聴制限を「公開」へ強制し、有効な間は公開以外への変更をAPIレベルで拒否する。転送の開始失敗はMSJP配信本体を巻き込まず縮退し、原因をライブチャットのシステムコメントで配信者へ通知する。あわせてOMEのOutputProfileへのAACレンディション追加に伴い、録画 (File publisher) は従来どおりのbypassトラックのみを明示指定して音声2トラック化を防ぐ
- Enhance: 配信アーカイブ用のGoogle連携 (Drive/YouTube) のトークン失効を早期検知して配信者へ告知するように (bsky-fork 独自)。従来は配信終了後のアーカイブ処理で初めて失効が発覚し (refresh token が Google に `invalid_grant` で拒否されると連携は自動解除されるが本人への告知が無かった)、アーカイブの保存に黙って失敗していた。今後は (1) refresh 拒否による連携自動解除の瞬間に Misskey 通知で再連携を促し、(2) 配信開始直後にも配信ごとに1回アクセストークンを検証・refresh して、失効している場合はライブチャットへシステム警告を投稿する (アーカイブが失われる前に配信者が再連携できる余地を作る)。なお Google Cloud Console 側で OAuth クライアントが「テスト中」ステータスの場合、refresh token は7日で強制失効するため (今回の失効の推定原因)、恒久対策にはアプリの「本番」公開が必要
- Feat: 配信アーカイブの YouTube「12時間上限」仕様による録画喪失を防ぐ多層対策を追加 (bsky-fork 独自)。YouTube は動画長が12時間を超えるアップロードを事後に削除する仕様で、実際に13時間超の配信録画が YouTube へ正常にアップロードされ (videoId 発行・Misskey 側は成功記録) たにもかかわらず YouTube 側で事後に削除され、成功パスでは Google Drive コピーを作らずローカルファイルも削除していたため録画が復元不能になる事故が発生した。これを構造的に防ぐため (1) 配信終了時の録画長ゲート: remux 後に ffprobe で実メディア長を測定し (失敗時は配信時間から推定)、12時間以上なら YouTube へのアップロードを行わず Google Drive のみへ保存する (`youtubeUploadStatus='skipped'`)。これにより「YouTube に送っても事後削除される録画」の発生を未然に防止。(2) 失敗アーカイブの7日間ローカル保持: Drive/YouTube いずれの永続コピーも確保できなかった場合 (Drive 容量不足・未連携など) 従来はローカル mp4 を即削除していたが、今後は新カラム `recordingRetentionExpiresAt` で管理し TNAS 上に7日間保持する。期限到達後は日次ジョブ `recordingRetentionCleanup` がファイルを削除。(3) YouTube アーカイブの事後削除検知: 6時間ごとのジョブ `youtubeHealthCheck` が `youtubeUploadStatus='ready'` の動画の生存を YouTube Data API (`videos.list`) で確認し、削除済みであれば `'unavailable'` に更新する (Drive コピーがあれば Drive 再生へフォールバック、なければアーカイブ利用不可を表示)。従来は Misskey 側が成功記録のまま死んだ動画IDを指し続け、壊れたプレイヤーが表示されていた。(4) 配信中の事前警告: 配信中、経過時間が11時間を超えると当該配信のライブチャットに5分間隔でシステム警告 (`source='system'` のサーバー生成コメント) を投稿し、配信者に12時間上限と「超えるとDrive保存のみ・Drive容量不足時は録画喪失」の旨を通知する (配信者は配信中チャットしか見ないため通知ではなくチャットへ投稿)。YouTube アップロード有効時のみ動作し、Drive 専用配信は12時間制限の対象外のため警告しない。あわせて失敗アーカイブのリカバリ用に `twitch/streams/retry-drive-upload` エンドポイント、および録画ダウンロード用のストリーム配信ルート `/recording-download/:streamId` を新規追加
- Enhance: 配信アーカイブ (Google Drive/YouTube) の録画パイプラインを実際に起動するように (bsky-fork 独自)。これまでは OME (自己配信基盤) 側で録画ファイルが生成されず機能が休眠していたが、OME の録画 REST API (`:startRecord`/`:stopRecord`) を配信開始/終了検知から明示的に呼び出す実装を追加し、実配信でのアーカイブ生成・YouTube アップロードまで動作することを確認した
- Fix: OME (自己配信) でビットレート超過により強制切断された配信者が、同じ streamKey で即座に再接続すると OME が普通に受理してしまい、高負荷配信によるサーバー負荷を実質的に防げていなかった問題を修正 (bsky-fork 独自)。これまではブラックリスト (Redis) を Misskey 側のライブ検知ポーリングでのみ参照しており、ingest 自体を止める手段がなかった。OME の AdmissionWebhooks (`/ome/admission`) を有効化し、ブラックリスト中の streamKey への配信開始要求を ingest 開始時点で拒否するようにした
- Fix: OME (自己配信) で OBS 配信を開始してもチャンネルがライブ表示にならず視聴プレイヤーも起動しない問題を修正 (bsky-fork 独自)。AdmissionWebhooks を使わない構成にもかかわらずライブ開始を検知する経路が実装されておらず、DB にライブセッション (source=ome) が作られていなかった。OmeStreamMonitorService の 10 秒ポーリングで OME の listStreams() を突合し、publish 中の streamKey に対応するチャンネルのライブセッションを自動作成 (+ フォロワー通知) するようにした (offline 化は既存の reconcile が担当)
...
- Fix: 配信コメントの翻訳判定で、絵文字だけのコメント (`:test:` のようなカスタム絵文字ショートコードや Unicode 絵文字のみ) が英語コメントと誤判定されて翻訳キューに積まれる問題を修正 (bsky-fork 独自)。言語判定 (`detectJaEn`) の前にカスタム絵文字ショートコード (`:[a-zA-Z0-9_+-]+:`) と Unicode 絵文字 (異体字セレクタ・ZWJ 含む) を除去し、除去後に意味のある文字が残らない場合は判定不能 (翻訳しない) とするようにした
- Feat: ライブチャンネルにオフライン画像 (`offlineImageId`) を保存できるように (bsky-fork 独自)。`live-channels/show` レスポンスに `offlineImageUrl` を追加し、`live-channels/update` で banner と同じ要領で設定・解除できる (DriveFile 参照、所有者チェック付き)
